...
First, an initiator generates `x` and sends a message frame containing `g^x,
...
The prime used is borrowed from [RFC5114 Section 2.3][ietf-dh] and reproduced
here:
...
bit key and 32 bit salt.  Our inputs to PBKDF2 use sha512-hmac for the PRF, an
iteration count of 5000, and 20 octet output. The PBDKDF2 salt is the 32-byte
SHA2-512 hash of the password while the PBKDF2 password is the password.
